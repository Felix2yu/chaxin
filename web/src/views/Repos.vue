...
async function pollSync() {
  try {
    ...
    syncStatus.progress = s.total > 0 ? Math.round((s.page / s.total) * 100) : 0
    if (!s.running) {
      stopPoll()
      await load()
            ...
            <td class="px-3 py-3.5">
              <div class="flex items-center gap-1">
                ...
                <button
                  ...
                  class="p-1.5 rounded-lg text-muted hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-500/10 transition-all"
                  title="删除"
                >
                  <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
