...
- **同步 Star 仓库**：一键异步拉取认证用户 Star 的全部仓库并做差异同步——新增 / 信息更新 / 无变化分别统计，前端实时显示进度条；已取消 Star 的仓库自动移除（手动添加或仍在监控中的仓库保留）
...
- **RSS 订阅**：提供 `/feed`（或 `/rss`）RSS 2.0 订阅源，聚合所有被监控仓库的最新版本发布，可在任意 RSS 阅读器中订阅
...
`http://<你的地址>/feed`（别名 `/rss`）提供 RSS 2.0 订阅源，内容为所有被监控仓库的最新版本（含更新日志摘要），按发布时间倒序排列。在设置页可查看并复制订阅地址，将其添加到任意 RSS 阅读器（如 Feedly、Inoreader 或自建 Miniflux 等）即可。订阅源基于监控检查时缓存的最新版本数据，不会额外请求 GitHub API。
...
| GET | `/feed`（或 `/rss`） | RSS 2.0 订阅源，聚合被监控仓库的最新版本发布 |
